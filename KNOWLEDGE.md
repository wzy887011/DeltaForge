...
| 环境访问审计 | `cloud-agent/environment_audit.sh` + `environment_audit_report.py` | 外部记录文件、网络、ioctl、内核调用、静态候选及关键SO哈希/版本；schema 2按六类检测面聚合 | 不修改返回值；ptrace时序污染必须单独标注 |
...
58 项代码表与 40 项 BSS 表分别执行均可存活；零间隔连续执行会退出，而同一 PID
