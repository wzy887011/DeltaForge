...
**原理:** 五层运行环境管理 — seccomp-bpf(内核) → maps管理 → libc 拦截(30+函数) → 代码调整(119处) → 外部守护。
...
L2 系统层 (属性/cpu/GPU)   →  33个假文件memfd + GPU inline branch 调整
L3 Java层 (Build/Prop)    →  JNI覆写16字段+4原生方法 + __system_property_get 70+属性
L4 内核层 (/proc/sys探测)  →  47+隐藏路径 + seccomp-bpf v4 (信号only)
L5 运行时层 (检测+kill)    →  119处调整 + exit_group地址检测

DeltaForge 五层:
  L1 seccomp-bpf v4    (constructor 49) — kernel层信号管理, 23条指令
  L2 maps自隐藏         (constructor 50+101) — madvise + r_debug link_map摘除
  L3 libc PLT 拦截      (加载瞬间) — 30+函数: 文件/属性/信号/链接器/网络/GPU
  L4 运行时调整         (constructor 150) — 检测链6节点 inline 调整
  L5 外部mem调整        (forge.c) — /proc/PID/mem 119处 + 1s分层守护
```
...
- **调整:** 67 代码段 + 40 BSS + 6 UE4, safe_write32(随机延时+回读+重试)

### libforgehook.c — 进程内拦截核心
- **加载:** 替换游戏 libtdmqimei.so (library替换), 游戏启动自动加载
- **Constructor链:** 48(probe)→49(seccomp)→50(maps hide)→100(chainload)→101(r_debug)→120(GPU)→150(检测链调整)
...
- **GPU 调整:** glGetString→Adreno 740, eglQueryString→Qualcomm (inline branch)
- **r_debug 摘除:** 从 linker 模块链表删除加载库条目
- **maps 过滤:** /proc/self/maps 动态过滤返回 memfd

### injector.c — ptrace加载器
- **用途:** 替换失败时的兜底方案, 或游戏运行后补加载
